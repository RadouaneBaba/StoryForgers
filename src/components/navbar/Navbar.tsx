...
export default function Navbar({user}: {user: KindeUser | null}) {
  return (
    <nav className="top-0 shadow bg-slate-900 md:flex md:items-center md:justify-between fixed w-full h-14 md:h-20">
      ...
      {/* Nav items and links */}
      <ul className="hidden md:flex md:items-center z-[-1] md:z-auto md:static left-0 md:w-auto w-full md:py-0 py-4 md:pl-0 pl-5 top-[-400px] transition-all ease-in duration-500">
      ...
      {
        user ? (
          <div className="flex items-center pr-2 md:pr-4 transition-all ease-in duration-500">
            <div>{user.given_name}</div>
            <LogoutLink className="text-slate-900 bg-slate-100 hover:bg-slate-50 rounded-md p-2 mx-6 font-bold">Logout</LogoutLink>
          </div>
        ) : (
          <div className="pr-2 md:pr-4 transition-all ease-in duration-500">
